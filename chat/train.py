...
def main():
    ...
    with training_args.main_process_first(desc="Log a few random samples from the raw training set"):
        for index in random.sample(range(len(raw_datasets["train_fit"])), 3):
    ...
    if training_args.do_train:
        column_names = list(raw_datasets["train_fit"].features)
    else:
    ...
    if training_args.do_train:
        if "train_fit" not in tokenized_datasets:
            raise ValueError("--do_train requires a train dataset")
        train_dataset = lm_datasets["train_fit"]
    ...
    if training_args.do_train:
        ...
        trainer.log_metrics("train_fit", metrics)
        trainer.save_metrics("train_fit", metrics)
        trainer.save_state()
